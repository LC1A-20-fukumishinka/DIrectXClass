...
bool ParticleManager::InitializeGraphicsPipeline()
{
	...
	//pipelineSet = std::make_unique<PipeClass::GSPipelineSet>();
	//// 頂点シェーダの読み込みとコンパイル
	...
	//}
PipelineDesc desc;
desc.VSname = L"Resources/shaders/particleVS.hlsl";
desc.PSname = L"Resources/shaders/particlePS.hlsl";
...
desc.isDepthWrite = false;
	...
	pipelineSet = BaseGraphicsPipeline::CreatePipeLine(desc, L"Resources/shaders/particleGS.hlsl");
	return true;
}
...
void ParticleManager::Draw()
{
	...
	myD->GetCommandList()->DrawInstanced((UINT)std::distance(particles_.begin(), particles_.end()), 1, 0, 0);
}
	...
	float start_scale, float end_scale, const XMFLOAT4 &start_color, const XMFLOAT4 &end_color)
{
	...
	p.e_scale = end_scale;
	p.s_color = XMLoadFloat4(&start_color);
	p.e_color = XMLoadFloat4(&end_color);
}

void ParticleManager::SetTexture(int textureHandle)
{
