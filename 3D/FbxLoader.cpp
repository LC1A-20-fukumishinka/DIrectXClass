...
#pragma region FBXノードのメッシュ情報を解析
	//FBXノードのローカル移動情報
...
void FbxLoader::LoadTexture(FbxModel *model, const std::string &fullpath)
{
	...
	model->material.textureHandle = TextureMgr::Instance()->SpriteLoadTexture(wfilepath);
}
...
{
	//スキニング情報
	FbxSkin *fbxSkin = static_cast<FbxSkin *>(fbxMesh->GetDeformer
	(0,
		FbxDeformer::eSkin));
	//スキニング情報がなければ終了
	if (fbxSkin == nullptr)
	...
	//ボーンの数
	int clsuterCount = fbxSkin->GetClusterCount();
	bones.reserve(clsuterCount);

	//全てのボーンについて
	for (int i = 0; i < clsuterCount; i++)
	{
		//FBXボーン情報
