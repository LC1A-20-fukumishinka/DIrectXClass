...
{
	XMVECTOR distV = sphere2.center - sphere1.center;
	float dist = XMVector3Dot(distV, distV).m128_f32[0];
	...
	{
		dist = sqrtf(dist);
		float cos = CosineTheorem(dist, sphere1.radius, sphere2.radius);
		float length = cos * sphere1.radius;
		distV = XMVector3Normalize(distV);
		distV *= length;
		*inter = (sphere1.center + distV);
	}
		return true;
}
