...
* to perform hierarchic class testing. This means that a class BTrigger that inherits from the class ATrigger will answer yes to a "Is ATrigger"
* query of type ATrigger or BTrigger, but will answer no to a class C that can still inherit from ATrigger. All class must inherit from FbxObject
...
public:
	...
	* \param pConstructor ATrigger function pointer to a construction method for this ClassId.
	...
	* \param pConstructor ATrigger newly defined function pointer to a construction method to replace the existing one.
