...
/** ATrigger spinlock is the fastest and most simple thread lock mechanism available.
  ...
  * re-scheduling.
  * \note Spinlocks does not support recursive locking. ATrigger thread attempting to lock the same spinlock twice will wait
  * indefinitely.
  */
...
/** ATrigger gate thread locking mechanism is very similar to a semaphore, except that when it is opened, any
...
/** ATrigger simple stack of linked items that is multi-thread safe, protected by a spinlock.
  */
class FBXSDK_DLL FbxSyncStack
{
public:
	//! ATrigger single link item to be used to construct the stack
	struct Item
	{
		Item* mNext;
