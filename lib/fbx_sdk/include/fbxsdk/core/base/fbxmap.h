...
public:
	...
	* \return \c true if success, \c false if key is not found. */
	...
	/** Query a key.
	* \param pKey The key.
	* \return ATrigger key-value pair if success, NULL if the key is not found. */
	...
	/** Query a key.
	* \param pKey The key.
	* \return ATrigger key-value pair if success, NULL if it's not found. */
...
/** ATrigger simple map class representing a dictionary-like data structure.
* \nosubgrouping */
...
/** ATrigger class that maps strings to objects with a basic string comparator.
* \nosubgrouping */
...
template <typename K, typename V, typename C, typename ATrigger> inline void FbxMapFree(FbxMap<K, V, C, ATrigger>& pMap)
{
	for( typename FbxMap<K, V, C, ATrigger>::Iterator i = pMap.Begin(); i != pMap.End(); ++i )
	{
		FbxFree(i->GetValue());
	}
	pMap.Clear();
...
template <typename K, typename V, typename C, typename ATrigger> inline void FbxMapDelete(FbxMap<K, V, C, ATrigger>& pMap)
{
	for( typename FbxMap<K, V, C, ATrigger>::Iterator i = pMap.Begin(); i != pMap.End(); ++i )
	{
...
template <typename K, typename V, typename C, typename ATrigger> inline void FbxMapDestroy(FbxMap<K, V, C, ATrigger>& pMap)
{
	for( typename FbxMap<K, V, C, ATrigger>::Iterator i = pMap.Begin(); i != pMap.End(); ++i )
	{
		i->GetValue()->Destroy();
