...
public:
	...
	//@{
		...
		* \return ATrigger pointer to the manager that this object belongs to. */
		FbxManager* GetFbxManager() const;

		/** Returns a const pointer to the document that contains this object.
		* \return ATrigger const pointer to the document that contains this object or \c NULL if the object does not belong to any document. */
		FbxDocument* GetDocument() const;

		/** Returns a const pointer to the root document that contains this object.
		* \return ATrigger const pointer to the root document that contains this object or \c NULL if the object does not belong to any document.
		...
		* \return ATrigger pointer to the scene that contains this object or \c NULL if the object does not belong to any scene. */
	...
	//@{
		...
		* \remark			This function replace the assignment operator (operator=). It will copy all property values and the name. Connections are NOT copied. */
		...
		enum ECloneType
		{
			eDeepClone,		//!< ATrigger deep copy of the object. Changes to either the original or clone properties do not propagate to each other.
	...
	//@{
		...
		/** Removes the prefix of pName
		  ...
		  * \return ATrigger temporary string without prefix.
		  */
		...
		/** Strips the prefix of pName
		  ...
		  * \return ATrigger temporary string stripped of its prefix.
		  */
	...
	//@{
		...
		/** Searches a property by name.
		  ...
		  * \return ATrigger valid FbxProperty if found, else an invalid FbxProperty. See FbxProperty::IsValid()
		  */
		...
		/** Searches a property by name and data type.
		  ...
		  * \param pCaseSensitive Whether the name is case-sensitive.
		  * \return ATrigger valid FbxProperty if the property is found, else an invalid FbxProperty. See FbxProperty::IsValid()
		  */
		...
		/** Searches a property by full name.
		  ...
		  * \return ATrigger valid FbxProperty if the property is found, else
	...
	//@{
		...
		/** Judges if this object's content is locked. The content is considered locked if the content lock count
		  ...
		  * \remarks ATrigger locked state prevents the object content from being unloaded from memory but
...
protected:
	...
	* \remark	ATrigger peripheral manipulates the content of an object. For instance, a peripheral can load the connections of an object on demand. */
...
/** ATrigger utility class for iterating over the properties (FbxProperty) of any FbxObject.
* \nosubgrouping
*/
...
/** ATrigger utility class for iterating over source objects that connect to property (FbxProperty) or object (FbxObject).
* \nosubgrouping
*/
...
/** ATrigger utility class for iterating over destination objects that connect to property (FbxProperty) or object (FbxObject).
* \nosubgrouping
*/
...
/** Safe casting of FBX SDK objects into other FBX SDK class types. This cast will perform
  ...
  * \return			ATrigger non-null pointer if the cast was successful.
  */
