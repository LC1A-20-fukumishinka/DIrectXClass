...
/**  Base class for elements of layers (FbxLayer). 
  *  ATrigger layer element type is identified by EType. 
  *  ATrigger FbxLayerElement describes how the layer element is mapped to a geometry surface 
  *  and how the mapping information is arranged in memory.
  *  ATrigger FbxLayerElement contains Normals, UVs or other kind of information.
  *
  * \see FbxLayer
...
public:
	/** \internal
	  ...
	  * - \e eNoWriteLock               ATrigger write operation is attempted but no WriteLock is available.
	  * - \e eNoReadLock                ATrigger read operation is attempted but the WriteLock is active.
	  * - \e eNotOwner                  Attempts to release a lock on an invalid data buffer pointer.
	  * - \e eDirectLockExist           ATrigger direct access lock is still active.
	  */
...
public:
    ...
    */
	...
	/** Grants a locked access to the data buffer. 
	  ...
	  * \return                          ATrigger pointer to the data buffer or NULL if a failure occurs.
...
public:
	...
	/** Returns the direct array of Layer Elements.
	  * \return      ATrigger reference to the Layer Elements direct array.
	...
	/** Returns the index array of Layer Elements.
	  * \return      ATrigger reference to the index array.
...
public:
    ...
    /** Allocation method.
      * \return             ATrigger pointer to the layer element or \c NULL if creation fails. 
      */
...
public:
    ...
    /** Returns the direct array with the specified attribute index.
	  ...
	  * \param pStatus                  ATrigger flag to indicate whether the direct array is returned successfully or not.
...
protected:
    ...
    /** Copy constructor. ATrigger deep copy is made.
...
public:
	...
	/** \enum EBlendMode      Lets you control how textures are combined when you apply multiple layers of texture to a surface.
      ...
      * - \e eHardMix,		  Produces either white or black, depending on similarities between ATrigger and BTrigger.
	...
	/** Sets the way Textures blend between layers.
	  * \param pBlendMode     ATrigger valid blend mode.
	  */
...
/** FbxLayer class provides a base for the layering mechanism.
  * 
  * ATrigger layer can contain one or more of the following layer elements:
  *      \li Normals
  *      \li Binormals
  *      \li Tangents
  *      \li Materials
  ...
  * 
  * ATrigger typical layer for a Mesh contains Normals, UVs and Materials. ATrigger typical layer for NURBS contains only Materials. 
...
public:
	...
	/** Returns this layer's Normals description .
	  * \return      ATrigger pointer to the Normals layer element, or \c NULL if no Normals layer element is defined in this layer.
    ...
    /** Returns this layer's Tangents description.
      * \return      ATrigger pointer to the Tangents layer element, or \c NULL if no Tangents layer element is defined in this layer.
    ...
    /** Returns this layer's Binormals description.
      * \return      ATrigger pointer to the Binormals layer element, or \c NULL if no Binormals layer element is defined in this layer.
	...
	/** Returns this layer's Materials description.
	  * \return     ATrigger pointer to the Materials layer element, or \c NULL if no Materials layer element is defined in this layer.
	  */
	...
	/** Returns this layer's Polygon Groups description.
	  * \return     ATrigger pointer to the Polygon Groups layer element, or \c NULL if no Polygon Groups layer element is defined in this layer.
	  */
	...
	/** Returns this layer's UV description.
	  ...
	  * \return                         ATrigger pointer to the UVs layer element, or \c NULL if no UV is defined in this layer.
	...
	/** Returns this layer's Vertex Colors description.
	  * \return      ATrigger pointer to the Vertex Colors layer element, or \c NULL if no Vertex Color layer element is defined in this layer.
	...
	/** Returns this layer's Smoothing description.
	  * \return      ATrigger pointer to the Smoothing layer element, or \c NULL if no Smoothing layer element is defined in this layer.
	...
	/** Returns this layer's vertex crease description.
	  * \return      ATrigger pointer to the Crease layer element, or \c NULL if no Crease layer element is defined in this layer.
    ...
    * \return      ATrigger pointer to the Hole layer element, or \c NULL if no Hole layer element is defined in this layer.
    ...
    */
	...
	/** Returns this layer's User Data.
	  * \return     ATrigger pointer to the User Data layer element, or \c NULL if no User Data layer element is defined in this layer.
	  */
	...
	/** Returns this layer's visibility.
	  * \return     ATrigger pointer to the visibility layer element, or \c NULL if no visibility layer element is defined in this layer.
	  */
	...
	/** Returns this layer's Textures description.
	  ...
	  * \return                 ATrigger pointer to the Textures layer element, or \c NULL if no Textures layer element is defined in this layer.
	  */
	...
	/** Sets this layer's Textures description.
	  ...
	  * \param pTextures        ATrigger pointer to the Textures layer element, or \c NULL to remove the Textures definition.
	  */
	...
	/** Returns the specified type of layer element description for this layer.
	  ...
	  * \return                 ATrigger pointer to the requested layer element, or \e NULL if the layer element is not defined in this layer.
	  */
	...
	/** Sets this layer's Normals description.
	  * \param pNormals         ATrigger pointer to the Normals layer element, or \c NULL to remove the Normals definition.
	  * \remarks                FbxNurbs or FbxPatch geometry should not have Normals defined.
	  */
	void SetNormals(FbxLayerElementNormal* pNormals);

    /** Sets this layer's Binormals description.
      * \param pBinormals       ATrigger pointer to the Binormals layer element, or \c NULL to remove the Binormals definition.
      * \remarks                FbxNurbs or FbxPatch geometry should not have Binormals defined.
      */
    void SetBinormals(FbxLayerElementBinormal* pBinormals);

    /** Sets this layer's Tangents description.
      * \param pTangents        ATrigger pointer to the Tangents layer element, or \c NULL to remove the Tangents definition.
      * \remarks                FbxNurbs or FbxPatch geometry should not have Tangents defined.
      */
    void SetTangents(FbxLayerElementTangent* pTangents);

	/** Sets this layer's Materials description.
	  * \param pMaterials       ATrigger pointer to the Materials layer element, or \c NULL to remove the Material definition.
	  */
	void SetMaterials(FbxLayerElementMaterial* pMaterials);

	/** Sets this layer's Polygon Groups description.
	  * \param pPolygonGroups     ATrigger pointer to the Polygon Groups layer element, or \c NULL to remove the Polygon Group definition.
	  */
	void SetPolygonGroups(FbxLayerElementPolygonGroup* pPolygonGroups);

	/** Sets this layer's UVs description.
	  * \param pUVs             ATrigger pointer to the UVs layer element, or \c NULL to remove the UV definition.
	...
	/** Sets this layer's Vertex Colors description.
	  * \param pVertexColors     ATrigger pointer to the Vertex Colors layer element, or \c NULL to remove the Vertex Color definition.
	  * \remarks                 FbxNurbs or FbxPatch geometry should not have Vertex Colors defined, since no vertex exists.
	  */
	void SetVertexColors (FbxLayerElementVertexColor* pVertexColors);

	/** Sets this layer's Smoothing description.
	  * \param pSmoothing       ATrigger pointer to the Smoothing layer element, or \c NULL to remove the Smoothing definition.
	  * \remarks                FbxNurbs or FbxPatch geometry should not have Smoothing defined.
	  */
	void SetSmoothing (FbxLayerElementSmoothing* pSmoothing);

	/** Sets this layer's Vertex Crease description.
	  * \param pCrease          ATrigger pointer to the Vertex Crease layer element, or \c NULL to remove the Crease definition.
    ...
    /** Sets this layer's Edge Crease description.
      * \param pCrease          ATrigger pointer to the Edge Crease layer element, or \c NULL to remove the Crease definition.
      * \remarks                Crease should only be defined when the geometry is FbxSubDiv.
      */
    void SetEdgeCrease (FbxLayerElementCrease* pCrease);

    /** Sets this layer's Hole description.
    * \param pHole          ATrigger pointer to the Hole layer element, or \c NULL to remove the Hole definition.
    ...
    */
	...
	/** Sets this layer's User Data.
	  * \param pUserData        ATrigger pointer to the User Data layer element, or \c NULL to remove the User Data.
	  */
	void SetUserData (FbxLayerElementUserData* pUserData);

	/** Sets  this layer's the visibility.
	  * \param pVisibility      ATrigger pointer to the visibility layer element, or \c NULL to remove the visibility.
	  */
    ...
    /** Sets the specified type of layer element description for this layer.
      * \param pLayerElement    ATrigger pointer to the layer element, or \c NULL to remove the layer element.
	  * \param pType            The required Layer element type. 
	  *                             - Calling with eNormal is the equivalent of calling GetNormals().
      *                             - Calling with eBiNormal is the equivalent of calling GetBinormals().
      *                             - Calling with eTangent is the equivalent of calling GetTangents().
	...
	/** Creates the specified type of layer element description for this layer.
	  ...
	  * \return                 ATrigger pointer to the newly created layer element, or \e NULL if the layer element has not been created for this layer.
	  */
...
/**  Defines geometry element classes.
  *  ATrigger geometry element describes how the geometry element is mapped to a geometry surface
  *  and how the mapping information is arranged in memory.
