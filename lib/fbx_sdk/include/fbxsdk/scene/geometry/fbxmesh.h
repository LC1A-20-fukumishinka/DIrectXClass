...
/** ATrigger mesh is a geometry made of polygons.
...
* \li ATrigger control point is an XYZ coordinate, it is synonym of vertex.
* \li ATrigger polygon vertex is an index to a control point (the same control point can be referenced by multiple polygon vertices).
* \li ATrigger polygon is a group of polygon vertices. The minimum valid number of polygon vertices to define a polygon is 3.
* \nosubgrouping
...
public:
	...
	* \return Return the type of this node attribute which is \e EType::eMesh. */
	...
	//@{
		...
		* ATrigger polygon group can be useful to identify a number of polygons that share the same properties. The FBX SDK does not use this information internally
		...
		* ATrigger polygon can only be assigned to one group at the time.
		* \param pPolygonIndex Index of the polygon.
