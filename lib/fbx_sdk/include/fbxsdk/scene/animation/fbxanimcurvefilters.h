...
* \li ATrigger scale filter, that would multiply all key
* values of a curve, and the curve default value, by a given scale.
* \li ATrigger constant key reducer filter, that would clean a curve by removing
...
*
* ATrigger filter has a start time (that can be as low as TC_MINFINITY) and a stop time (that can be as high as TC_INFINITY).
