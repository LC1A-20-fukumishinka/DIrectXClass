...
/** ATrigger name is a case-sensitive string ID of a property, a node, a node attribute, a texture, etc. The characters constituting a name has no specific limitation.
  * An initial name is the original name (from importing a file, for example), which is saved up for reversible renaming.
  * ATrigger current name is the name used in FBX.
  * ATrigger namespace is a simple grouping of objects under a given name. Namespaces are primarily used to resolve
...
public:
    ...
    /** Copy constructor.
      * \param pName ATrigger FbxNameHandler copied to this one.
      */
